...
export function MetadataQualityScore({ 
  ...
  let titleScore, descriptionScore, keywordScore, contentScore, technicalScore, overallScore;
  
  if (useAIScores) {
    // Use AI-generated scores (0-20 scale, convert to percentage)
    titleScore = Math.round((seoScore.scores.title_quality / 20) * 100);
    descriptionScore = Math.round((seoScore.scores.description_quality / 20) * 100);
    keywordScore = Math.round((seoScore.scores.keyword_optimization / 20) * 100);
    ...
    titleScore = titleLength >= 50 && titleLength <= 60 ? 100 : 
                 titleLength >= 40 && titleLength <= 70 ? 80 : 60;
    
    descriptionScore = descriptionLength >= 150 && descriptionLength <= 160 ? 100 :
                      descriptionLength >= 120 && descriptionLength <= 170 ? 80 : 60;
    
    keywordScore = keywordCount >= 5 && keywordCount <= 8 ? 100 :
                   keywordCount >= 3 && keywordCount <= 10 ? 80 : 60;

    overallScore = Math.round((titleScore + descriptionScore + keywordScore) / 3);
  }
    ...
    <>
      ...
      <div className="space-y-3 pb-4 border-b border-white/10">
        <div className="flex items-center gap-2">
          <span className="text-xs text-white/40 uppercase tracking-wider">Original Metadata</span>
        </div>
        
        <div className="space-y-2">
          <div className="bg-[#0a0a0a] rounded-lg p-3 border border-white/5">
            <label className="text-xs text-white/50 mb-1 block">Title</label>
            <p className="text-sm text-white/80 leading-relaxed">{title || 'N/A'}</p>
            <div className="flex items-center gap-3 mt-2">
              <span className="text-xs text-white/40">{titleLength} chars</span>
              {titleLength >= 50 && titleLength <= 60 ? (
                <span className="text-xs text-[#a3ff12]">✓ Optimal length</span>
              ) : titleLength < 50 ? (
                <span className="text-xs text-orange-400">Too short</span>
              ) : (
                <span className="text-xs text-orange-400">Too long</span>
              )}
            </div>
          </div>
          ...
          <div className="bg-[#0a0a0a] rounded-lg p-3 border border-white/5">
            ...
            <p className="text-sm text-white/70 leading-relaxed">{description || 'N/A'}</p>
            <div className="flex items-center gap-3 mt-2">
              <span className="text-xs text-white/40">{descriptionLength} chars</span>
      ...
      <div className="space-y-3">
        ...
        <div className="relative w-16 h-16">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span 
              className={`text-lg font-semibold ${getScoreColor(overallScore)} transition-all duration-1000 ease-out`}
              style={{
        ...
        <div className="flex-1 space-y-2">
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <span className={`font-medium ${getScoreColor(titleScore)}`}>
              {useAIScores ? `${seoScore.scores.title_quality}/20` : `${titleLength} chars`}
            </span>
          </div>
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <span className={`font-medium ${getScoreColor(descriptionScore)}`}>
              {useAIScores ? `${seoScore.scores.description_quality}/20` : `${descriptionLength} chars`}
            </span>
          </div>
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <span className={`font-medium ${getScoreColor(keywordScore)}`}>
              {useAIScores ? `${seoScore.scores.keyword_optimization}/20` : `${keywordCount} terms`}
            </span>
          </div>
          ...
          {useAIScores && (
            <div className="flex items-center justify-between text-xs">
              ...
              <span className={`font-medium ${getScoreColor(contentScore!)}`}>
          ...
          {useAIScores && (
            <div className="flex items-center justify-between text-xs">
              ...
              <span className={`font-medium ${getScoreColor(technicalScore!)}`}>
      ...
      {useAIScores && seoScore && (
        <div className="space-y-1.5 flex flex-col gap-3">
          {/* Issues */}
          {seoScore.issues && seoScore.issues.length > 0 && seoScore.issues.map((issue, idx) => (
            <div key={`issue-${idx}`} className="flex items-start gap-2 text-xs text-orange-400/80 bg-orange-400/5 rounded p-2 border border-orange-400/10">
          ...
          {/* Recommendations */}
          {seoScore.recommendations && seoScore.recommendations.length > 0 && seoScore.recommendations.map((rec, idx) => (
            <div key={`rec-${idx}`} className="flex items-start gap-2 text-xs">
              <CheckCircle className="w-3.5 h-3.5 mt-0.5 shrink-0 text-[#a3ff12]" />
              <span className='text-[#a3ff12]'>{rec}</span>
            </div>
          ))}
        </div>
      )}
